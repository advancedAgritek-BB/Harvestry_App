...
public class CreateBatchCodeRuleRequestValidator : AbstractValidator<CreateBatchCodeRuleRequest>
{
    private static readonly string[] ValidResetPolicies = { "never", "annual", "monthly", "per_harvest" };

    public CreateBatchCodeRuleRequestValidator()
    {
        RuleFor(x => x.Name)
        ...
        RuleFor(x => x.ResetPolicy)
            ...
            .Must(policy => ValidResetPolicies.Contains(policy?.ToLowerInvariant()))
